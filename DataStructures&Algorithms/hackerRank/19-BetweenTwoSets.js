...
function getTotalX(array1, array2) {


  let array1Highest = Math.max(...array1);
  let array2Lowest = Math.min(...array2);
  console.log(array1Highest);

  let factors = [];

  for (let i = array1Highest; i <= array2Lowest; i++) {
    if (
      array1.every((num) => i % num === 0) &&
      array2.every((num) => num % i === 0)
    ) {
      factors.push(i);
    }
  }
